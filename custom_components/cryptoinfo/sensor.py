    ...
    def _update(self):
        ...
        try:
            if price_data:
                ...
                self._high_timestamp = r.json()[0]["ath_date"][0:19]
                # high_dt = r.json()[0]["ath_date"][0:19].strptime("%Y-%m-%dT%H:%M:%S")
            ...
            self._high = None
            self._high_timestamp = None
           # self._high_timestamp = datetime.datetime(1970,1,1).strftime("%d-%m-%Y %H:%M")
